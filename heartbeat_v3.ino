...
bool runConfigPortalForced() {
 ...
 WiFi.disconnect(false, false);
  ...
  if (CFG.wifiSsid.length()) strncpy(ssidBuf, CFG.wifiSsid.c_str(), sizeof(ssidBuf)-1);
if (CFG.wifiPass.length()) strncpy(passBuf, CFG.wifiPass.c_str(), sizeof(passBuf)-1);
...
if (CFG.endpoint.length() == 0) {
  ...
  // 3) kalau gagal -> buka portal untuk input ulang ssid/pass + endpoint tetap
