...
function handleGetActions() {
    ...
    $stmt = $db->prepare("
        INSERT INTO device (devices_id, status, last_sent) 
...
function handleLog() {
    ...
    // Insert log
    $stmt = $db->prepare("
        INSERT INTO action_logs (actionid, devices_id, message, version, logged_at)
        VALUES (:actionid, :devices_id, :message, :version, NOW())
    ");
    $stmt->execute([
        'actionid' => $actionId,
        'devices_id' => $deviceId,
 ...
 * POST body: {"id": "phinet-xxxx", "device_id": "name", "ssid": "...", "rssi": -42, "v": "1.0"}
 */
function handleHeartbeat() {
    ...
    $stmt = $db->prepare("
        INSERT INTO heartbeats (devices_id, heartbeaton, ipaddress, useragent)
        VALUES (:devices_id, NOW(), :ipaddress, :useragent)
    ");
    $stmt->execute([
        'devices_id' => $hwId,
        'ipaddress' => $clientIp,
 ...
 * POST body: {"devices_id": "phinet-xxxx", "action": "update", "value": "https://...", "additional": {}}
 */
function handleCreateAction() {
    $input = getJsonInput();
    
    if (empty($input['devices_id']) || empty($input['action'])) {
        jsonResponse(['error' => 'Missing required fields'], 400);
    }
    
    $deviceId = $input['devices_id'];
    $action = $input['action'];
    $value = $input['value'] ?? '';
    ...
    $stmt = $db->prepare("SELECT devices_id FROM device WHERE devices_id = :id");
    $stmt->execute(['id' => $deviceId]);
    if (!$stmt->fetch()) {
    ...
    // Create action
    $stmt = $db->prepare("
        INSERT INTO device_actions (devices_id, action, value, additional, status, created_at)
        VALUES (:devices_id, :action, :value, :additional, 'pending', NOW())
    ");
    $stmt->execute([
        'devices_id' => $deviceId,
        'action' => $action,
    ...
    jsonResponse([
        'success' => true,
        'actionid' => $actionId,
        'message' => "Action '$action' created for device $deviceId"
    ]);
}
...
function handleGetDeviceActions() {
    $input = getJsonInput();
    $deviceId = $input['devices_id'] ?? ($_GET['devices_id'] ?? '');
    
    if (empty($deviceId)) {
        jsonResponse(['error' => 'Missing devices_id'], 400);
    }
    ...
    $stmt = $db->prepare("
        ...
        LIMIT 50
    ");
    $stmt->execute(['devices_id' => $deviceId]);
